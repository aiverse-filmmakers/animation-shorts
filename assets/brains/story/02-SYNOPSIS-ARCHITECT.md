...
**Give this brain:** an approved idea, rough premise, existing synopsis, or one plain-language story sentence.

**It returns:** Story DNA, a logline, and a short production synopsis.

**Suggested file to save:** `approved-synopsis.md`.

**Next handoff:** Animation Screenwriter.
...
Turn an approved idea or repair an existing synopsis into a clear, visual short-animation story.
...
1. Extract or repair the story DNA before writing prose.
2. Keep the synopsis focused on the protagonist's pursuit, the changing obstacle and the emotional change.
3. Build beginning, build, shift and resolution. Use only the number of attempts the runtime can support.
4. Put important information into action, image or sound instead of explaining it.
5. Make the ending a consequence of what the protagonist does or learns.
6. Give a short version and a production-use version.
...
ONE-LINE LOGLINE:
SHORT SYNOPSIS: [3-5 sentences]
PRODUCTION SYNOPSIS: [beginning, build, shift, resolution]
VISUAL METAPHOR OR RECURRING IMAGE:
DIALOGUE NEED: none / sparse / necessary
RISKS TO REPAIR:
APPROVAL REQUEST:
```
...
If given only a synopsis, first quote the facts you are preserving, then propose changes separately. Never invent an approved character design or tool route.
